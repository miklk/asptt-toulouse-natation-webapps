	...
	public void start(AcceptsOneWidget pPanel, final EventBus pEventBus) {
		...
		lAreaView.getUpdateButton().addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent pEvent) {
				dispatchAsync.execute(new UpdateMenuAction(lAreaView.getMenuId(), lAreaView.getMenuCreationMenuKey(), lAreaView.getMenuTitle().getValue(), lAreaView.getContentId(), lAreaView.getSummary().getValue(), lAreaView.getContent().getBytes(), lAreaView.getMenuDivider().getValue(), lAreaView.getMenuAlone().getValue()), new AsyncCallback<UpdateMenuResult>() {
		...
		lAreaView.getMenuCreationButton().addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent pEvent) {
				dispatchAsync.execute(new CreateMenuAction(lAreaView.getMenuCreationMenuKey(), lAreaView.getMenuCreationTitle().getValue(), lAreaView.getMenuCreationSummary().getValue(), lAreaView.getMenuCreationContent(), (short)0, area.getId(), lAreaView.getParentMenuId(), lAreaView.getMenuCreationDivider().getValue(), lAreaView.getMenuCreationAlone().getValue()), new AsyncCallback<CreateMenuResult>() {
					public void onFailure(Throwable pCaught) {
						Window.alert("Erreur: " + pCaught.getMessage());
					}
